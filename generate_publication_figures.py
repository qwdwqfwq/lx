...
model_colors_map = {
    ...
    "Independent SOC/SOE": '#8C564B', # Brown
    "N-BEATS": '#9467BD', # muted purple
    "CNN-GRU": '#E377C2', # raspberry
    "LSTM": '#7F7F7F', # medium gray
    "TCN-LSTM": '#BCBD22', # yellow-green
    "BiGRU": '#17BECF', # cyan
    "Informer": '#87CEEB', # Light Blue (SkyBlue)
    "TabPFN": '#C5B0D5', # lavender
}
...
def main():
    ...
    gs = fig.add_gridspec(4, 2, hspace=0.6, wspace=0.3) # 调整为4行2列，增大垂直间距
    ...
    for dataset_type in datasets_to_plot:
        ...
        for metric_prefix, pred_title, err_ylabel, pred_ylabel in metrics_to_plot:
            ...
            for model_name in selected_models_for_ts_plot:
                ...
                if f"{model_name}_{dataset_type}" in all_loaded_data:
                    ...
                    # 预测图平滑窗口设置为 65
                    current_ax_pred.plot(current_times, smooth_data(pred_metric, window_length=65), 
            ...
            current_ax_pred.legend(loc='upper right', fontsize=8, frameon=False, ncol=2, bbox_to_anchor=(1.0, 1.02)) # 调整图例位置，避免遮挡
            ...
            current_ax_err.set_ylim(-10, 10) # 误差图纵坐标改为-10到10
            ...
            if dataset_type == "LA92":
                zoom_start_default = 4500 
                zoom_length = 1000 
            elif dataset_type == "UDDS":
                zoom_start_default = 5000 
                zoom_length = 800
                ...
                for model_name in selected_models_for_ts_plot:
                    ...
                    if f"{model_name}_{dataset_type}" in all_loaded_data:
                        ...
                        if len(pred_metric_zoom) > 0:
                            y_min_zoom = min(y_min_zoom, np.min(smooth_data(pred_metric_zoom, window_length=65))) 
                            y_max_zoom = max(y_max_zoom, np.max(smooth_data(pred_metric_zoom, window_length=65))) 

                        axins.plot(current_times[(current_times >= zoom_start) & (current_times <= zoom_end)], 
                                   smooth_data(pred_metric[(current_times >= zoom_start) & (current_times <= zoom_end)], window_length=65), 
                                   color=model_plot_color, linewidth=1.8, label=model_name) 
        ...
        # 由于不再使用颜色循环，移除此行
        # row_offset += 2 # This was incorrectly placed here, it should not be here. The loop structure has changed.
