...
from electrochemical_46features_datamodule import Electrochemical46FeaturesDataModule # 导入使用StandardScaler的数据模块
...
class TrueSOTAElectrochemicalKANTransformer(nn.Module):
    ...
    def __init__(self, input_dim, num_heads, num_layers, num_outputs, 
        ...
        self.fusion_weight_head_param = nn.Parameter(torch.tensor(0.5)) # ͷʵȨ
...
class TrueSOTAElectrochemicalPhysicsLoss(nn.Module):
    ...
    def _compute_electrochemical_loss(self, predictions, targets, inputs):
        ...
        if inputs.shape[-1] > 6 and self.feature_scaler is not None: # 确保有内阻特征和scaler
            ...
            # 对内阻进行逆标准化，得到原始物理值
            # 注意：这里我们使用的是feature_scaler，它现在是StandardScaler
...
class TrueSOTAElectrochemicalLightningModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        x, y, _ = batch # 数据模块现在只返回3个元素：x, y, original_end_indices
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        x, y, original_end_indices = batch # 接收原始结束索引
        y_hat = self.forward(x)
...
def main():
    ...
    try:
        ...
        datamodule = Electrochemical46FeaturesDataModule(args) # 使用StandardScaler的数据模块
